...
NAVIGATION = [
    ...
    ("Achat", [("📥", "BL réception"), ("📡", "DESADV achat"), ("⚖️", "Écarts achat")]),
    ("Vente", [("📤", "BL expédition"), ("📡", "DESADV vente"), ("⚖️", "Écarts vente")]),
...
def appliquer_ecran_defaut(vue_id: str, cles: list[str]) -> None:
    """À la première ouverture de la vue dans la session (reconnexion),
    applique l'écran marqué par défaut, s'il existe."""
    drapeau = f"ecran_defaut_fait_{vue_id}"
    ...
    if defauts is None or defauts.empty:
        return
    try:
...
def render_dashboard() -> None:
    ...
    # Heatmap façon « contributions GitHub » : une case par jour (colonnes =
    # semaines, lignes = jours), vert d'autant plus foncé qu'il y a de BL.
    tmp = df.dropna(subset=["date_reception"]).copy()
    tmp["d"] = pd.to_datetime(tmp["date_reception"])
    calendrier = pd.DataFrame({"d": pd.date_range(dmin, dmax, freq="D")})
    ...
    heatmap = (
        alt.Chart(par_jour).mark_rect(cornerRadius=3, width=16, height=16,
                                      stroke="#FFFFFF", strokeWidth=2).encode(
            x=alt.X("Semaine:O", title=None,
                    axis=alt.Axis(labelExpr="timeFormat(toDate(datum.value), '%d/%m')",
                                  labelAngle=0, labelFontSize=11)),
            y=alt.Y("Jour:N", sort=jours_fr, title=None,
                    axis=alt.Axis(labelFontSize=11)),
            color=alt.Color("BL:Q", title="BL / jour",
                            scale=alt.Scale(range=["#EBEDF0", "#C6E48B", "#7BC96F",
                                                   "#43B02A", "#196127"]),
...
def vue_ecarts(sens: str) -> None:
    ...
    try:
        bl_sans, desadv_sans = repository.lire_ecarts(sens)
    except Exception as e:
        st.error(f"Erreur de lecture de la base : {e}")
        st.stop()
...
elif vue == "Écarts achat":
    vue_ecarts(repository.SENS_ACHAT)
elif vue == "Écarts vente":
