...
SELECT recid, issuedatetime, CAST(integrationdate AS DATE) AS integrationdate, numero_bl
FROM (
...
SELECT numero_bl, nom_fournisseur, issuedatetime, integrationdate FROM (
  SELECT e.numero_bl, c.supplier AS nom_fournisseur,
         e.issuedatetime, e.integrationdate,
...
desadv = [(r["numero_bl"], r["nom_fournisseur"], r["issuedatetime"], r["integrationdate"])
...
with conn:
    with conn.cursor() as cur:
        ...
        nb_cli = cur.rowcount
        # base_desadv (sens ACHAT) : additif et idempotent. numero_bl unique par
        # sens -> ON CONFLICT (numero_bl, sens) DO NOTHING (préserve d'éventuelles
        # corrections manuelles ; pas de doublon de numéro de BL).
        cur.executemany(
            f"INSERT INTO {pg_schema}.base_desadv "
            "(numero_bl, nom_fournisseur, sens, issuedatetime, integrationdate) "
            "VALUES (%s, %s, 'ACHAT', %s, %s) ON CONFLICT (numero_bl, sens) DO NOTHING",
            desadv,
        )
...
print(f"Lakebase synchronisé : +{nb_frs} fournisseur(s), +{nb_cli} client(s), "
      f"+{nb_desadv} DESADV achat (lignes déjà présentes ignorées).")
