"""Application « Administration des BL » — V4.

Expérience structurée type « model-driven » (à la Power Apps) :
  - logo eMotors et navigation latérale par sections (modules en intitulés,
    vues en boutons toujours visibles) ;
  - tableau de bord interactif (KPI + graphiques filtrables) ;
  - ruban d'actions contextuel au-dessus de chaque grille, activé dès le
    clic sur une ligne (sélection par cellule OU cases à cocher) ;
  - visionneuse d'images plein format (zoom, rotation, impression,
    téléchargement) ;
  - DESADV avec l'état du message EDI (colonne, filtre, KPI) ;
  - référentiels en CRUD complet, dont Adresses et Sites logistiques.
"""
...
# Navigation : (section, [(icône, vue), ...]) — toutes les vues sont toujours
# visibles (aucun module à déplier), comme dans les consoles modernes.
NAVIGATION = [
    ...
    ("Gestion", [("👤", "Gestionnaires"), ("💼", "Portefeuilles"), ("🚪", "Quais"),
                 ("📍", "Adresses"), ("🏢", "Sites logistiques"), ("🔔", "Notifications")]),
]
...
# =====================================================================
# NAVIGATION LATÉRALE — sections texte + vues toujours visibles
# =====================================================================
st.session_state.setdefault("nav_vue", VUE_DASHBOARD)

with st.sidebar:
    ui.afficher_logo()
    st.divider()
    for section, vues in NAVIGATION:
...
# =====================================================================
# BOÎTES DE DIALOGUE (fiche BL, visionneuse, confirmation de suppression)
# =====================================================================
...
def vue_bl(nom_vue: str, types: list[str]) -> None:
    ...
    # --- Grille : un clic sur une ligne (n'importe quelle cellule) la
    # sélectionne et active le ruban ; les cases à cocher permettent la
...
# =====================================================================
# VUE « DESADV » — filtres + KPI EDI + grille éditable
# =====================================================================
def vue_desadv(sens: str) -> None:
    ...
    with st.expander("🔍 Filtres", expanded=False):
        ...
        f_sedi = c3.selectbox("État EDI", ["Tous", "OK", "EDI NOK"], key=f"dsedi_{suffixe}")
        if achat:
            gest_options = [""] + repository.lister_gestionnaires()
            f_gest = c3.selectbox("Gestionnaire", gest_options,
    ...
    try:
        df = repository.lire_desadv(
            sens, f_num, f_frs, f_gest, f_dmin, f_dmax,
            statut_edi="" if f_sedi == "Tous" else f_sedi).reset_index(drop=True)
    except Exception as e:
        st.error(f"Erreur de lecture de la base : {e}")
        st.stop()
...
# =====================================================================
# VUE « SITES LOGISTIQUES » — filtres + grille éditable (comme Portefeuilles)
# =====================================================================
