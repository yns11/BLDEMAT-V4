...
def lister_tous_tiers() -> list[str]:
    ...
    s = get_settings()
    df = _run(f"SELECT name FROM {s.pg_schema}.base_tiers ORDER BY name", fetch=True)
    return df["name"].tolist() if df is not None else []
...
def lister_adresses() -> list[str]:
    s = get_settings()
    df = _run(f"SELECT adresse FROM {s.pg_schema}.adresses ORDER BY adresse", fetch=True)
...
def adresses_par_tiers() -> dict[str, str]:
    ...
    df = _run(
        ...
        "GROUP BY entite", fetch=True)
    if df is None or df.empty:
        return {}
    return dict(zip(df["entite"], df["adresse"]))
...
def roles_utilisateur(utilisateur: str) -> list[str]:
    ...
    si aucun rôle. Lève une exception si la table n'existe pas encore
    (l'appelant décide alors du mode ouvert)."""
    s = get_settings()
    df = _run(
...
@st.cache_data(ttl=120, show_spinner=False)
def rbac_actif() -> bool:
    """Le RBAC s'active dès que la table roles_utilisateurs contient au moins
    une ligne ; table vide ou absente = mode ouvert (accès complet)."""
    s = get_settings()
    try:
        df = _run(f"SELECT 1 FROM {s.pg_schema}.roles_utilisateurs LIMIT 1", fetch=True)
...
def bls_desadv_pour_tiers(tiers: str, sens: str, limite: int = 200) -> list[str]:
    ...
    df = _run(
        ...
        "WHERE nom_fournisseur = %(t)s AND sens = %(sens)s "
...
def inserer_bl(
    ...
    _auditer(id_bl, "CREATION", utilisateur, apres=numero_bl)
...
def rechercher_bl(
    ...
    df = _run(
        ...
        SELECT id_bl, numero_bl, date_reception, plage_horaire, nom_fournisseur, quai_reception,
               ...
               type_operation, est_supprime, desadv_rapproche
...
def mettre_a_jour_bl(id_bl: str, champs: dict, utilisateur: str) -> None:
    ...
    if not a_modifier:
        return
    s = get_settings()
    # Valeurs actuelles pour l'audit (avant -> après par champ modifié).
    avant = _run(
        f"SELECT {', '.join(a_modifier)} FROM {s.pg_schema}.suivi_bl "
        "WHERE id_bl = %(id)s", params={"id": id_bl}, fetch=True)
    ...
    if avant is not None and not avant.empty:
        ligne = avant.iloc[0]
        for champ, nouveau in a_modifier.items():
            ancien = ligne[champ]
            if str(ancien if ancien is not None else "") != str(nouveau if nouveau is not None else ""):
                _auditer(id_bl, "MODIFICATION", utilisateur, champ,
                         ancien, nouveau)
...
def supprimer_bl(id_bl: str, utilisateur: str) -> None:
    ...
    _auditer(id_bl, "SUPPRESSION", utilisateur)
...
def restaurer_bl(id_bl: str, utilisateur: str) -> None:
    ...
    _auditer(id_bl, "RESTAURATION", utilisateur)
...
def _auditer(id_bl: str, evenement: str, utilisateur: str, champ: str | None = None,
    ...
    """Journalise dans audit_bl — best effort : ne bloque jamais l'opération
    métier (ex. table absente avant la migration V6)."""
    s = get_settings()
    try:
        _run(
...
def lire_ecarts(sens: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Écarts BL ⇄ DESADV d'un sens : (BL sans DESADV, DESADV sans BL).
    Comparaison insensible à la casse sur le numéro de BL."""
    s = get_settings()
    ...
    bl_sans = _run(
        ...
        """,
        params={"types": list(types), "sens": sens}, fetch=True)
    desadv_sans = _run(
        f"""
...
def fraicheur_desadv(sens: str) -> dict:
    ...
    df = _run(
        ...
        f"FROM {s.pg_schema}.base_desadv WHERE sens = %(sens)s",
    ...
    try:
        for ligne in lignes:
            _run(
                ...
                "VALUES (%(u)s, %(n)s, %(c)s, %(ia)s, %(v)s, %(i)s)",
                params={"u": ligne.get("utilisateur"), "n": ligne.get("numero_bl"),
                        "c": ligne["champ"], "ia": ligne.get("valeur_ia"),
                        "v": ligne.get("valeur_validee"), "i": bool(ligne["identique"])},
            )
    except Exception as e:
...
def sauver_ecran(utilisateur: str, vue: str, nom: str, etat: str,
    ...
    if est_defaut:                                  # un seul défaut par vue
        _run(f"UPDATE {s.pg_schema}.ecrans_utilisateur SET est_defaut = false "
             "WHERE lower(utilisateur) = lower(%(u)s) AND vue = %(v)s",
             params={"u": utilisateur, "v": vue})
    _run(
        f"INSERT INTO {s.pg_schema}.ecrans_utilisateur (utilisateur, vue, nom, est_defaut, etat) "
        "VALUES (%(u)s, %(v)s, %(n)s, %(d)s, %(e)s) "
        "ON CONFLICT (utilisateur, vue, nom) "
        "DO UPDATE SET etat = EXCLUDED.etat, est_defaut = EXCLUDED.est_defaut",
        params={"u": utilisateur, "v": vue, "n": nom, "d": est_defaut, "e": etat})
