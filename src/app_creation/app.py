...
from bl_core import extraction, images, rbac, repository, ui
from bl_core.identity import get_current_user

try:
    # File d'attente hors-ligne (localStorage) — optionnelle : sans la
    # dépendance, l'app fonctionne normalement (sans sauvegarde navigateur).
...
# =====================================================================
# FILE D'ATTENTE HORS-LIGNE — pages scannées conservées dans le navigateur
# (localStorage) : si le réseau coupe en quai, elles sont restaurées à la
# reconnexion et l'enregistrement peut être rejoué. Taille limitée par le
# quota du navigateur (~5 Mo) : au-delà, la sauvegarde est simplement ignorée.
# =====================================================================
...
def demander_sauvegarde_navigateur() -> None:
    """À appeler après chaque changement des pages : l'écriture localStorage
...
def sync_navigateur() -> None:
    """Écrit (ou efface) les pages en localStorage si demandé."""
    ...
    if st.session_state.pages:
        ...
        expr = (f"try {{ localStorage.setItem('{CLE_LS}', {json.dumps(contenu)}) }} "
                "catch (e) { console.warn('BL hors-ligne :', e) }")
    else:
        expr = f"localStorage.removeItem('{CLE_LS}')"
...
def restaurer_depuis_navigateur() -> None:
    ...
    if (streamlit_js_eval is None or st.session_state.pages
        ...
        return
    brut = _js(f"localStorage.getItem('{CLE_LS}')", "ls_lecture")
    if not brut:
        return
    try:
...
# =====================================================================
if etape == 1:
    # RBAC : seules les opérations autorisées par les rôles sont proposées
    # (matrice bl_core/rbac.py ; mode ouvert tant que la table est vide).
    ctx_rbac = rbac.contexte_rbac(get_current_user())
...
# =====================================================================
    ...
    if col_suiv.button("Suivant ➡️", type="primary", use_container_width=True):
        if not numero.strip():
            ...
            # Qualité IA : journalise « valeur IA vs valeur validée » champ
            # par champ (une fois par BL), pour mesurer la précision.
            if (st.session_state.extraction_statut == "ok"
                    and not st.session_state.get("qualite_journalisee")):
                _journaliser_qualite(ex, numero.strip(), fournisseur, statut,
                                     date_reception, commentaire.strip(), tous_tiers)
...
# =====================================================================
    ...
    # BL en base : la sauvegarde hors-ligne du navigateur n'a plus lieu d'être.
    _js(f"localStorage.removeItem('{CLE_LS}')",
