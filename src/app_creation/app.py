...
# =====================================================================
    ...
    with col_msg:
        if statut_ia == "ok" and any(ex.values()):
            ...
            st.info("🔎 L'IA n'a rien pu extraire de l'image : saisie manuelle.")
    ...
    def _detecte(champ: str, reconnu: bool = False) -> None:
        val = ex.get(champ)
        if val:
            st.caption(f"🔎 détecté : « {val} »" + (" ✓ reconnu" if reconnu else ""))

    # --- Numéro de BL ---
    numero = st.text_input("Numéro du BL *",
    ...
    # --- Tiers : DESADV auto -> IA rapprochée -> sélection manuelle ---
    frs_desadv = None
    ...
    if frs_desadv:
        ...
        # Rapprochement du tiers détecté par l'IA avec le référentiel.
        tiers_reconnu = extraction.rapprocher(ex.get("tiers", ""), tous_tiers)
        if ex.get("tiers"):
            if tiers_reconnu:
                st.caption(f"🔎 détecté : « {ex['tiers']} » ✓ reconnu : {tiers_reconnu}")
            else:
                st.caption(f"🔎 détecté : « {ex['tiers']} » — non trouvé au référentiel, "
                           "choisissez manuellement.")

        filtre_frs = st.text_input("Filtrer la liste", value="",
    ...
    if avec_plage_quai:
        ...
        if ex.get("quai"):
            st.caption(f"🔎 détecté : « {ex['quai']} »" + (" ✓ reconnu" if quai_reconnu else ""))
    else:
        quai = None

    # --- État de réception (texte manuscrit, réception seule) ---
    if avec_statut:
        ex_nok = "nok" in ex.get("statut", "").lower()
        defaut_statut = (repository.STATUT_EDI_NOK if donnees.get("statut") == repository.STATUT_EDI_NOK
                         or (donnees.get("statut") is None and ex_nok) else repository.STATUT_OK)
        choix = st.radio("État de réception *", ["OK", "EDI NOK"],
                         index=0 if defaut_statut == repository.STATUT_OK else 1, horizontal=True)
